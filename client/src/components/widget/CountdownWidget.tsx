...
export function CountdownWidget({ widget, tasks }: Props) {
  const { t } = useLanguage();
  ...
  if (!targetDate) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">No date set for this task</p>
    ...
    <div className="text-center">
      ...
      {task.completed ? (
        <div className="py-6">
          ...
          <p className="text-lg font-semibold text-green-600 dark:text-green-400">{t.completed}!</p>
        </div>
      ) : (
        <div className="py-4">
          <div className={`grid grid-cols-3 gap-4 ${isPast ? "opacity-60" : ""}`}>
            {[{ label: "Days", val: days }, { label: "Hours", val: hours }, { label: "Mins", val: minutes }].map(({ label, val }) => (
          ...
          {isPast && <p className="text-sm text-red-600 dark:text-red-400 mt-3 font-medium">{t.overdueTasks}</p>}
        </div>
      )}
    </div>
  );
